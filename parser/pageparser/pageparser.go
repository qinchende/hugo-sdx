...
import (
	"bytes"
	"github.com/gohugoio/hugo/aaasdx"
	"io"
	"io/ioutil"
...
// add by sdx on 2020-12-25
func ParseSdx(r io.Reader, cfg Config, relativePrefix string) (Result, error) {
	return parseSectionSdx(r, cfg, lexIntroSection, relativePrefix)
}
...
// SDX:important very. TODO：非常重要，这个地方开始读取.md文件了。
...
// add by sdx on 2020-12-25
func parseSectionSdx(r io.Reader, cfg Config, start stateFunc, relativePrefix string) (Result, error) {
	b, err := ioutil.ReadAll(r)
	if err != nil {
		return nil, errors.Wrap(err, "failed to read page content")
	}
	// 改变图片路径，方便显示
	// ++ Add by sdx on 2020-12-24
	b = aaasdx.SdxChangeImagePath(b, relativePrefix)
	// ++ Add by sdx on 2020-12-24
	return parseBytes(b, cfg, start)
}
